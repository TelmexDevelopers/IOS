...
function ImprimeCombo($numero,$seleccion)
{
	if ($numero)
	{
		switch($numero){
			...
			break;
			case 18:
				$tabla = "vw_subgerente_entrega";	
				$moo = 'class="mootools combos_busqueda"';	
				$nombre_combo = "subgerente_entrega";
				$id='id_Usuario';
				$string = 'str_Nombre_Subgerente';
				$campos = $id.",".$string;			
				$where = "";
				$orderby=" ORDER BY str_Nombre_Subgerente ASC";
			break;
			case 19:
		...
		if (!$RS) die("No Recordset");
		  while(!$RS->EOF)
		  {
			  ...
			  $html .= '<option value=';
			  if ($tabla == "vw_subgerente_entrega" || $tabla == "vw_supervisores" || $tabla == "cat_con_ot" || $tabla == "cat_area_responsable" || $tabla == "cat_fase_ios" || $tabla == "vw_combo_cns" || $tabla == "VW_SUPERVISORES_ANALISIS" || $tabla == "VW_IPE_ANALISIS" || $tabla == "vw_CM_Mantto" || $tabla == "vw_ipe_entrega" || $tabla == "vw_supervisores_entrega")
			  {
