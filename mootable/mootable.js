...
var MooTable = new Class({
	...
	_initDisplayOptions: function(){
		...
		this.formOptions = new Element('form').setProperty('id','formOptions').setProperty('name','formOptions');
		this.form = this.formOptions.injectInside( this.displayOptions );
		
		
		this.displayTableOptions = new Element('table').setProperty('border','0').setProperty('cellspacing','1').setProperty('bgcolor','#CCCCCC');
		this.form = this.displayTableOptions.injectInside( this.formOptions );
		
		
		
		
		this.displayTableOptions_TR = new Element('tr');
		this.form = this.displayTableOptions_TR.injectInside( this.displayTableOptions );
		...
		if (i==0 || i % 10 == 0 )
		{
			this.displayTableOptions_TD = new Element('td').setProperty('valign','top').setProperty('bgcolor','#fff');
			this.form = this.displayTableOptions_TD.injectInside( this.displayTableOptions_TR );
		}
			var id = 'mootable_h'+i;
...
//		var checked = this.form['mootable_h'+col].checked;
		var checked = $('mootable_h'+col).checked;
//		alert(checked);
		this.rows.each( function(row){
