...
GLOBAL $gSQLMaxRows,$gSQLBlockRows,$ADODB_ROUND;
	...
	for ($i=0; $i < $ncols; $i++) {	
		...
		$hdr .= "<TH bgcolor=\"#000066\" style=\"color:#ffffff\" class=\"Texto_Mediano_Blanco\">$fname</TH>";
	}
		//agregare
		//$hdr .= "<TH bgcolor=\"#000066\" style=\"color:#ffffff\" class=\"Texto_Mediano_Blanco\">Ver		</TH>";
	...
	//echo $bgcolor;
		$s .= "<tr valign=\"middle\" bgcolor=\"".$bgcolor."\" class=\"Texto_Mediano_Gris\">\n";
		...
		for ($i=0; $i < $ncols; $i++) {
			...
			case 'T':
				if (empty($v)) $s .= "	<TD> &nbsp; </TD>\n";
				else $s .= "	<TD>".$rs->UserTimeStamp($v,"Y-m-d") ."</TD>\n";
			break;
			...
			*/
			...
			default:
				...
				if ($i == 0)
				{
					...
					$referencia = trim($v);
					//$D_referencia = trim($v);
					//on click a la referencia
//$v = trim("<a href=\"javascript:void(0);\" style=\"text-decoration: none;\">".$v."</a>");
		$v = trim("<a href=\"javascript:detalle_referencia('".$referencia."','".$id_tramos."');\" style=\"text-decoration: none;\">".$v."</a>");
				} else {
		...
		// on click a tramos
		//$s .= "	<TD><a href=\"javascript:busca_tramos('".$tramos."');\" style=\"text-decoration: none;\">Tramos</a></TD>\n";
